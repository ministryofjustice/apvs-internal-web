...
describe('Adding a new top up flow', () => {
  ...
  it('should approve the second post-approval check', async () => {
    await browser.url('/')
    ...
    try {
      check1Label = await $('#check-1-label')
      exists = await check1Label.isExisting()
      expect(exists, 'Check 1 should not be available after check 2 has been approved').to.equal(false)
    } catch (error) {
      log.error(error)
    }
    ...
    try {
      ...
      expect(exists, 'Check 2 should not be available after check 2 has been approved').to.equal(false)
    } catch (error) {
      log.error(error)
    }
  })
