...
describe('Adding a new top up flow', () => {
  ...
  it('should reject the first post-approval check', async () => {
    await browser.url('/')
    ...
    let checkDecision = await $('#check-decision')
    await checkDecision.selectByVisibleText('Reject')

    let checkComments = await $('#check-comments')
    await checkComments.setValue('Testing')

    let submitButton = await $('#complete-check-1')
    await submitButton.click()

    try {
      check1Label = await $('#check-1-label')
      exists = await check1Label.isExisting()
      expect(exists, 'Check 1 should not be available after check 1 has been Rejected').to.equal(false)
    } catch (error) {
      log.error(error)
    }
    ...
    try {
      check2Label = await $('#check-2-label')
      exists = await check2Label.isExisting()
      expect(exists, 'Check 2 should not be available after check 1 has been Rejected').to.equal(false)
    } catch (error) {
      log.error(error)
    }
  })
